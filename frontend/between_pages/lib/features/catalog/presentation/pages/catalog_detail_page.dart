...
class _CatalogDetailPageState extends ConsumerState<CatalogDetailPage> {
    ...
    switch (widget.type) {
      case CatalogItemType.book:
        ...
        return UnifiedCatalogData(
          ...
          description: b.description, genres: b.genres,
          pageCount: b.pageCount, publishYear: b.publishYear, publisher: b.publisher,
        );
      case CatalogItemType.manga:
        final m = _currentItem as MangaResponseDTO;
        return UnifiedCatalogData(
          title: m.title ?? 'Sin título', author: m.author ?? 'Autor desconocido',
          coverUrl: m.coverUrl, description: m.description, genres: m.genres,
          totalChapters: m.totalChapters, publicationStatus: m.publicationStatus,
          demographic: m.demographic, malScore: m.malScore,
        );
      case CatalogItemType.fanfic:
        final f = _currentItem as FanfictionResponseDTO;
        return UnifiedCatalogData(
          title: f.title ?? 'Sin título', author: f.author ?? 'Autor desconocido',
          coverUrl: f.coverUrl, description: f.description, genres: f.genres,
          totalChapters: f.totalChapters, publicationStatus: f.publicationStatus,
          mainShip: f.mainShip, theme: f.theme,
        );
    }
  }
  ...
  dynamic _getExistingJournal() {
    switch (widget.type) {
      case CatalogItemType.book:
        final b = _currentItem as BookResponseDTO;
        return ref.watch(journalEntryProvider((JournalType.book, b.idBook ?? 0)));
      case CatalogItemType.manga:
        final m = _currentItem as MangaResponseDTO;
        return ref.watch(journalEntryProvider((JournalType.manga, m.idManga ?? 0)));
      case CatalogItemType.fanfic:
        final f = _currentItem as FanfictionResponseDTO;
        return ref.watch(journalEntryProvider((JournalType.fanfic, f.idFanfic ?? 0)));
    }
  }
      ...
      switch (widget.type) {
        case CatalogItemType.book:
          ...
          await ref.read(bookJournalRepositoryProvider).saveRaw(
            BookJournalRecordDTO(userId: userId, bookId: book.idBook ?? 0,
                googleBooksId: book.googleBooksId, status: status, currentPage: 0).toJson(),
          );
        case CatalogItemType.manga:
          final m = _currentItem as MangaResponseDTO;
          final mangaId = m.idManga;
          await ref.read(mangaJournalRepositoryProvider).saveRaw(
            MangaJournalRecordDTO(userId: userId, mangaId: (mangaId != null && mangaId > 0) ? mangaId : null,
                malId: m.malId, status: status, currentChapter: 0).toJson(),
          );
        case CatalogItemType.fanfic:
          ...
          await ref.read(fanficJournalRepositoryProvider).saveRaw(
            FanficJournalRecordDTO(userId: userId, fanficId: f.idFanfic ?? 0, ao3Id: f.ao3Id, status: status, currentChapter: 0).toJson(),
          );
      }
      _invalidateJournals();
      if (!mounted) return;
    ...
    try {
      ...
      switch (widget.type) {
        case CatalogItemType.book:
          ...
          await ref.read(bookJournalRepositoryProvider).updateRaw(
            BookJournalRecordDTO(id: j.id, userId: userId, bookId: j.book.idBook ?? 0,
                googleBooksId: j.book.googleBooksId, status: newStatus, currentPage: j.currentPage).toJson(),
          );
        case CatalogItemType.manga:
          final j = existing as MangaJournalResponseDTO;
          await ref.read(mangaJournalRepositoryProvider).updateRaw(
            MangaJournalRecordDTO(id: j.id, userId: userId, mangaId: j.manga?.idManga,
                malId: j.manga?.malId, status: newStatus, currentChapter: j.currentChapter).toJson(),
          );
        case CatalogItemType.fanfic:
          final j = existing as FanficJournalResponseDTO;
          await ref.read(fanficJournalRepositoryProvider).updateRaw(
            FanficJournalRecordDTO(id: j.id, userId: userId, fanficId: j.fanfic.idFanfic ?? 0,
                ao3Id: j.fanfic.ao3Id, status: newStatus, currentChapter: j.currentChapter).toJson(),
          );
      }
      _invalidateJournals();
      if (!mounted) return;
    ...
    switch (widget.type) {
      case CatalogItemType.book:
        updated = await Navigator.push<BookResponseDTO>(
          context, MaterialPageRoute(builder: (_) => BookEditPage(book: item as BookResponseDTO)),
        );
      case CatalogItemType.manga:
        updated = await Navigator.push<MangaResponseDTO>(
          context, MaterialPageRoute(builder: (_) => MangaEditPage(manga: item as MangaResponseDTO)),
        );
      case CatalogItemType.fanfic:
        updated = await Navigator.push<FanfictionResponseDTO>(
          context, MaterialPageRoute(builder: (_) => FanficEditPage(fanfic: item as FanfictionResponseDTO)),
        );
    }
...
final lists = ref.read(listProvider);
    ...
    if (selectedList != null) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${_data.title} añadido a ${selectedList.name}')),
          );
        }
      } catch (e) {
        if (mounted) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_data.genres.isNotEmpty)
                        _GenresSection(genres: _data.genres, accent: _typeColor),
...
class _InfoSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (data.totalChapters != null) {
      rows.add(_InfoRowData(Icons.format_list_numbered_rounded, 'Capítulos', '${data.totalChapters}'));
    }
    if (data.pageCount != null && data.pageCount! > 0) {
      rows.add(_InfoRowData(Icons.auto_stories_rounded, 'Páginas', '${data.pageCount}'));
    }
    if (data.publishYear != null) {
      rows.add(_InfoRowData(Icons.calendar_today_rounded, 'Año', '${data.publishYear}'));
    }
    if (data.publisher != null && data.publisher!.isNotEmpty) {
      rows.add(_InfoRowData(Icons.business_rounded, 'Editorial', data.publisher!));
    }
    if (data.malScore != null && data.malScore! > 0) {
      rows.add(_InfoRowData(Icons.star_rounded, 'Score MAL', '${data.malScore}'));
    }
    if (data.demographic != null && data.demographic!.isNotEmpty) {
      rows.add(_InfoRowData(Icons.group_rounded, 'Demografía', data.demographic!));
    }
    if (data.mainShip != null && data.mainShip!.isNotEmpty) {
      rows.add(_InfoRowData(Icons.favorite_rounded, 'Ship principal', data.mainShip!));
    }
    ...
    if (data.publicationStatus != null && data.publicationStatus!.isNotEmpty) {
      rows.add(_InfoRowData(Icons.schedule_rounded, 'Estado publicación', data.publicationStatus!));
    }

    final desc = data.description;

    if (rows.isEmpty && (desc == null || desc.isEmpty)) return const SizedBox.shrink();
