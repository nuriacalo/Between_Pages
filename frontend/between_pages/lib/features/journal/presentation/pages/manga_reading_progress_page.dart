...
class _MangaReadingProgressPageState extends ConsumerState<MangaReadingProgressPage> {
          ...
          content: Column(
            ...
            children: [
              const Text(
                ...
                'necesitamos saber cuántos capítulos tiene este manga en total.',
              ),
  ...
  void _goToEditJournal() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => JournalItemEditPage(journal: _journal, type: JournalType.manga)),
    );
  }

  Widget _buildProgressTab(Color accent, int currentChapter, int? totalChapters, double progress, MangaJournalResponseDTO journal, ColorScheme colorScheme, TextTheme textTheme) {
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: FilledButton.icon(
              onPressed: () {
                context.push('/journal/manga/session', extra: _journal);
              },
  ...
  Widget build(BuildContext context) {
    ...
    final accent = const Color(0xFFE8A87C);

    final updatedJournal = ref.watch(
      journalEntryProvider((JournalType.manga, _manga.idManga ?? 0)),
    );
  ...
  Widget _buildStatsGrid(ColorScheme colorScheme, TextTheme textTheme, MangaJournalResponseDTO journal, int currentChapter, int? totalChapters) {
    int? days;
      ...
      builder: (context, ref, child) {
        ...
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return _StatCard(stat: stat, accent: const Color(0xFFE8A87C));
          },
        );
      },
    );
